string bigAdd(string num1,string num2){
	...
	while(index1>=0&&index2>=0){
		int cur=(num1[index1]+num2[index2]+upper)%10;
		upper=(num1[index1]+num2[index2]+upper)/10;
		index1--;index2--;
		resstack.push(cur+'0');
	}
	while(index1>=0){
		int cur=(num1[index1]+upper)%10;
		upper=(num1[index1]+upper)/10;
		index1--;
		resstack.push(cur+'0');
	}
	while(index2>=0){
		int cur=(num2[index2]+upper)%10;
		upper=(num2[index2]+upper)/10;
		index2--;
		resstack.push(cur+'0');
	}
	if(upper>0)
...
string multiply(string num,char c,int base){
	...
	for(int i=len-1;i>=0;i--){
		...
		resstack.push(tmp%10);
		upper=tmp/10;
	}
	if(upper>0)resstack.push(upper);
	ostringstream ostr;
	while(!resstack.empty()){
		ostr<<resstack.top();
		resstack.pop();
